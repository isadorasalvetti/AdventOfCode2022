...
monkeysRound 10000 _ allMonky counter = (counter, allMonky)
...
monkeyTurn (Monkey items op test friends) = do
    let newWorry = map (\x -> x `mod` mult) (map op items)
...
main = do
    ...
    let lastRound = monkeysRound 0 0 monkys startCounter
    let roundCounter = sortBy (\a b -> compare b a) (fst lastRound)
    print $ roundCounter!!0 * roundCounter!!1